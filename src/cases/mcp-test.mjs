...
async function runAgentWithTools(query, maxIterations = 30) {
  // state在多个Runnable之间传递，包含当前的消息列表messages、是否done以及最终的回复final和所有的tools等信息
  let state = {
  ...
  for (let i = 0; i < maxIterations; i++) {
    ...
    // 每一轮都通过一个完整的Runnable chain（LLM + 工具调用处理）
    state = await agentStepChain.invoke(state)
    if (state.done) {
      console.log(chalk.bgGreen(`✅ AI 已完成思考，最终回答: ${state.final}`));
      return state.final
    }
  }
...
await runAgentWithTools("北京昌平北站附近的酒店，最近的 3 个酒店，拿到酒店图片，打开浏览器，展示每个酒店的图片，每个 tab 一个 url 展示，并且在把那个页面标题改为酒店名");
